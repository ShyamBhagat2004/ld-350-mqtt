...
def send_keep_alive_command(dev, endpoint_out):
    send_command(dev, 0, endpoint_out[0], "KA")
...
try:
    while True:
        ...
        # Send keep alive command every 0.5 seconds
        send_keep_alive_command(dev, endpoint_out)
        #time.sleep(0.5)
