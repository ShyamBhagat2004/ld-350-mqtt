...
# Commands to be sent
commands = [
    b'RAW 1\r',    # Enable raw data output
    b'STATUS\r',   # Query status
]

# Send commands to the device
for command in commands:
    send_command(dev, endpoint_out_0, command)
    time.sleep(1)  # Wait a bit between commands to ensure the device processes them
...
try:
    while True:
        try:
            # Attempt to read data from both interfaces
            data_0 = dev.read(endpoint_in_0, 64, timeout=1000)
            data_1 = dev.read(endpoint_in_1, 64, timeout=1000)
            ...
            # Clear the buffer periodically for demonstration
            buffer.clear()
